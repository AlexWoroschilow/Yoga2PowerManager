...
class Sata(interface.Hardware):
    @property
    def is_powersave(self):
        for device in self.devices:
            if os.path.isfile(device):
                if 'min_power' not in self._run("cat %s" % device):
                    return False
        return True
    ...
    def devices(self):
        for path in glob.glob("/sys/class/scsi_host/host*"):
            if os.path.exists(path):
                yield "%s/link_power_management_policy" % path

    def powersave(self):
        for device in self.devices:
            if os.path.isfile(device):
                yield "echo 'min_power' > '%s';" % device

    def perfomance(self):
        for device in self.devices:
            if os.path.isfile(device):
                yield "echo 'medium_power' > '%s';" % device
